...
# famd_no_asd$eig %>% as_tibble() %>% mutate(row_num = row_number()) %>% 
...
! [Eigenvalues](./asd_fa_parellel_pc_screeplot.png)
...
famd_dim1 <- famd_no_asd$var$contrib %>% as_tibble() %>%
...
famd_dim2 <- famd_no_asd$var$contrib %>% as_tibble() %>%
	mutate(variable = rownames(famd_no_asd$var$contrib)) %>% 
	janitor::clean_names() %>%
	arrange(desc(dim_2)) %>%
	mutate(num_row = row_number())
...
famd_dim3 <- famd_no_asd$var$contrib %>% as_tibble() %>%
	mutate(variable = rownames(famd_no_asd$var$contrib)) %>% 
	janitor::clean_names() %>%
	arrange(desc(dim_3)) %>%
	mutate(num_row = row_number())
...
famd_dim4 <- famd_no_asd$var$contrib %>% as_tibble() %>%
	mutate(variable = rownames(famd_no_asd$var$contrib)) %>% 
	janitor::clean_names() %>%
	arrange(desc(dim_4)) %>%
	mutate(num_row = row_number())
